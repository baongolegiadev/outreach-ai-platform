...
- **Unique**: `(sequenceEnrollmentId, sequenceStepId)` tránh queue trùng cho cùng enrollment-step.
...
- Test outbound queue end-to-end (task 011): `scripts/test-outbound-mailer-queue.sh`
  - Script sẽ tự đọc `workspaceId` từ `scripts/workspace.json` (trừ khi bạn truyền `WORKSPACE_ID` để override).
